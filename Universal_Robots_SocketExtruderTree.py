...
DEFAULT_HEADER_SCRIPT = """
  ...
  # connect to PC
  socket_open("192.168.4.2", 23, "socket_01")
  sleep(2.0)
...
class RobotPost(object):
    ...
    PRINT_E_AO = 1 # Analog Output ID to command the extruder flow
    ...
    EXTRUDER_INDEX = 0
    LAST_EXTRUDER_INDEX = 0
    counter = 0

    # added by kaguya for 3d printing
    FEEDRATE_FIXED = 20 #feedrate mm/min
    ...
    # use this to calculate F, added by kaguya
    def new_move(self, pose2):
        ...
        def Calculate_Time(Dist, Vmax, Amax):
            ...
            if Dist <= 2*Xacc:
                ...
                Ttot = 2*tacc + Tvmax
            return Ttot
        # this e needs to divided by pai * r2 = 0.4158
        
        add_material = (self.PRINT_E_NEW - self.PRINT_E_LAST) * 0.4158 * 1.55
        if self.EXTRUDER_INDEX == 1:
            add_material = (self.PRINT_E_NEW - self.PRINT_E_LAST) * 0.4158 * 2.15
        

        # ring
        #add_material = (self.PRINT_E_NEW - self.PRINT_E_LAST) * 0.4158 * 0.5 * 0.8
        
        self.PRINT_E_LAST = self.PRINT_E_NEW
        
        # do not pumpback
        if add_material >= 0 and self.counter != 2:
            ...
            #extrusion
            #extrusion = time_s * self.FEEDRATE_FIXED / 60 

            # trans to URscript
            self.setSocket(add_material, feedrate)
        elif add_material < 0 and add_material > -5 and self.EXTRUDER_INDEX == self.LAST_EXTRUDER_INDEX and self.counter != 2:
            # pumpback 4.0
            add_material = 0.3
            time_s = 0.5
            feedrate = 60 * add_material / time_s
    ...
    def MoveL(self, pose, joints, conf_RLF=None):
        ...
        if self.USE_MOVEP:
            ...
            self.addline('movel(%s,accel_mss,speed_ms,0,%s)' % (target, 0))
        
        self.counter = self.counter + 1
    
    # fix f
    # def setSocket(self, socket_e):          
    #     if socket_e > 0:
    #         #transform to socket string (ascii)
    #         socket_e = str('%.3f' % socket_e)
    #     else:
    #         socket_e = "0"
    #     newline = 'socket_send_line("G1 E%s F%s", "socket_01")' % (socket_e, str('%.3f' % self.FEEDRATE_FIXED))
    #     self.addline(newline)

    #control layer by e, added by kaguya
    def setSocket(self, socket_e, socket_f):        
        #if socket_e >= 0 and socket_f >= 0:
            #transform to socket string 
        socket_e = str('%.3f' % socket_e)
        ...
        if self.EXTRUDER_INDEX == self.LAST_EXTRUDER_INDEX:
            newline = 'socket_send_line("G1 E%s F%s", "socket_01")' % (socket_e, socket_f)
            self.addline(newline)
            
        self.LAST_EXTRUDER_INDEX = self.EXTRUDER_INDEX
        ...
        #     self.addline(newline)
        #     # socket_ea = -socket_e
        #     # socket_fa = -socket_f / 2
        #     # socket_ea = str('%.3f' % socket_ea)
        #     # socket_fa = str('%.3f' % socket_fa)
        #     # #addin
        #     # newline = 'socket_send_line("G1 E%s F%s", "socket_01")' % (socket_ea, socket_fa)
        #     # self.addline(newline)
    ...
    def waitDI(self, io_var, io_value, timeout_ms=-1):
        ...
        self.addline('  sync()')
        self.addline('end')
    #added to set AO signal
    def setAO(self, io_var, io_value):          
        if type(io_value) != str: # set default variable value if io_value is a number            
            if io_value > 0:
                #transform to AO signal [0,1]
    ...
    def RunCode(self, code, is_function_call = False):
        ...
        if is_function_call:
            ...
            if code.startswith("Extruder0("):
                self.PRINT_E_NEW = float(code[10:-1])
                self.EXTRUDER_INDEX = 0
                # Skip the message call generation
                return

            if code.startswith("Extruder1("):
                self.PRINT_E_NEW = float(code[10:-1])
                self.EXTRUDER_INDEX = 1
                return
    ...
    # use this to recognize extruder
    def RunMessage(self, message, iscomment = False):
        """Show a message on the controller screen"""
        if iscomment:
            #add for extrusion output
            if message.startswith("Extruder0("):
                self.PRINT_E_NEW = float(message[10:-1])
                self.EXTRUDER_INDEX = 0
                # Skip the message call generation
                return

            if message.startswith("Extruder1("):
                self.PRINT_E_NEW = float(message[10:-1])
                self.EXTRUDER_INDEX = 1
                return
